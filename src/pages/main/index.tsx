...
const getData = (type)=>{
  ...
  postLogin('/users/login_ccnu', {
    ...
    console.log('登录成功');
    get(`/evaluations/list/all?cur_evaluation_id=0&limit=10&property=${classType}`).then((res1) => {
      // console.log(res.data);
      if(res1.code == 0){
        processData(res1.data).then(newData => {
          setComments(newData);
      }).catch(error => {
  ...
  return (
    <View className='main'>
      ...
      <View className='classLine'>
        <View className={'label' +' ' + (classType == 1 ?'active':'')}
          onClick={()=>{
        ...
        <View className={'label' +' ' + (classType == 2 ?'active':'')}
          onClick={()=>{
        ...
        <View className={'label' +' ' + (classType == 3 ?'active':'')}
          onClick={()=>{
        ...
        <View className={'label' +' ' + (classType == 4 ?'active':'')}
          onClick={()=>{
